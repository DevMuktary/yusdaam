...
export default function OwnerForgotPassword() {
  ...
  const handleRequestOtp = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch("/api/owner/auth/forgot-password", {
        method: "POST",
      ...
      <div className="lg:w-1/3 xl:w-1/4 bg-void-navy border-b lg:border-b-0 lg:border-r border-white/10 p-6 sm:p-10 lg:p-12 flex flex-col justify-between">
        <div>
          ...
          <div className="hidden sm:block mt-12">
            <Briefcase size={48} className="text-cobalt mb-6 opacity-70" />
            <h2 className="text-lg font-bold mb-2">Asset Owner Recovery</h2>
            <p className="text-xs text-slate-light leading-relaxed">
              Regain access to your Asset Administration Portal. Verify your identity with a secure one-time passcode sent directly to your registered email address.
            </p>
          </div>
        </div>
        ...
        <div className="max-w-md w-full">
          ...
          {step < 4 && (
            <div className="flex items-center justify-between mb-8 pb-4 border-b border-white/10 text-xs">
              <div className={`flex items-center gap-2 font-bold ${step >= 1 ? "text-cobalt-light text-crisp-white" : "text-slate-light/40"}`}>
                <span className={`w-6 h-6 rounded-full flex items-center justify-center text-[11px] ${step >= 1 ? "bg-cobalt text-white" : "bg-white/10 text-slate-light/40"}`}>1</span>
                <span>Email</span>
              </div>
              <div className={`h-0.5 flex-1 mx-3 ${step >= 2 ? "bg-cobalt" : "bg-white/10"}`} />
              <div className={`flex items-center gap-2 font-bold ${step >= 2 ? "text-cobalt-light text-crisp-white" : "text-slate-light/40"}`}>
                <span className={`w-6 h-6 rounded-full flex items-center justify-center text-[11px] ${step >= 2 ? "bg-cobalt text-white" : "bg-white/10 text-slate-light/40"}`}>2</span>
                <span>OTP</span>
              </div>
              <div className={`h-0.5 flex-1 mx-3 ${step >= 3 ? "bg-cobalt" : "bg-white/10"}`} />
              <div className={`flex items-center gap-2 font-bold ${step >= 3 ? "text-cobalt-light text-crisp-white" : "text-slate-light/40"}`}>
                <span className={`w-6 h-6 rounded-full flex items-center justify-center text-[11px] ${step >= 3 ? "bg-cobalt text-white" : "bg-white/10 text-slate-light/40"}`}>3</span>
                <span>Password</span>
              </div>
            </div>
          )}
          ...
          {step === 1 && (
            <div className="animate-in fade-in duration-300">
              <div className="mb-8 text-center lg:text-left">
                <h1 className="text-2xl sm:text-3xl font-black uppercase mb-2 tracking-wide text-crisp-white">
                  Reset Password
                </h1>
                <p className="text-sm text-slate-light">
                  Enter your registered asset owner email address and we will send a 6-digit security OTP.
                </p>
              </div>
              ...
              <div className="mt-8 pt-6 border-t border-white/10 text-center">
                ...
                >
                  <ArrowLeft size={14} /> Back to Owner Login
                </Link>
              </div>
            </div>
          )}
          ...
          {step === 3 && (
            <div className="animate-in fade-in duration-300">
              <div className="mb-8 text-center lg:text-left">
                ...
                <p className="text-sm text-slate-light">
                  Choose a secure password for your Asset Owner account.
                </p>
              </div>
          ...
          {step === 4 && (
            <div className="text-center py-6 animate-in zoom-in-95 duration-300">
              ...
              <p className="text-sm text-slate-light leading-relaxed mb-8 max-w-sm mx-auto">
                Your Asset Owner account password has been successfully updated. You can now log in using your new credentials.
              </p>
