      ...
      {signedAssets.length === 0 ? (
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          {signedAssets.map((vehicle) => {
            ...
            // DIRECT URL FETCH - No more string manipulation hacks
            // We use the new dedicated fields created on the Contract table
            // using ts-ignore just in case Prisma types haven't caught up in your IDE yet
            // @ts-ignore
            const hpaDownloadLink = contract.ownerHpaUrl 
              // @ts-ignore
              ? `${contract.ownerHpaUrl}?fl_attachment=YUSDAAM_HPA_${vehicle.registrationNumber}.pdf` 
              : null;
              
            // @ts-ignore
            const poaDownloadLink = contract.ownerPoaUrl 
              // @ts-ignore
              ? `${contract.ownerPoaUrl}?fl_attachment=YUSDAAM_POA_${vehicle.registrationNumber}.pdf` 
              : null;
            ...
            return (
              <div key={vehicle.id} className="bg-void-navy/50 border border-cobalt/20 rounded-xl p-6 shadow-lg hover:border-cobalt/50 transition duration-300 flex flex-col h-full">
                ...
                <div className="mt-auto">
                  ...
                  {/* @ts-ignore */}
                  {(!contract.ownerHpaUrl || !contract.ownerPoaUrl) && (
                    <p className="text-[10px] text-amber-400/70 mt-4 text-center italic">
                      * Final PDFs are currently being processed or uploaded.
                    </p>
                  )}
                </div>
