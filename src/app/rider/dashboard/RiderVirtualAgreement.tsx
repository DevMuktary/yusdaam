...
export default function RiderVirtualAgreement({ rider, vehicle, contract, guarantors }: { rider: any, vehicle: any, contract: any, guarantors: any[] }) {
  ...
  const handleSubmitAll = async () => {
    ...
    try {
      ...
      const opt = {
        margin: [0.4, 0.4, 0.4, 0.4], // Tight 0.4-inch margins
  ...
  const HpaDocument = ({ isPdf = false }: { isPdf?: boolean }) => {
    ...
    const textStyle = isPdf ? "text-[10px] leading-snug text-black font-serif text-justify" : "text-sm text-slate-light leading-relaxed font-sans text-justify";
    const headingStyle = isPdf ? "font-bold text-[11px] border-b border-black pb-0.5 mt-4 mb-2 text-black uppercase" : "font-bold text-crisp-white text-base mt-8 border-b border-cobalt/20 pb-2 uppercase";
    const paraSpacing = isPdf ? "mb-2" : "mb-4";
    const listSpacing = isPdf ? "mb-3 space-y-0.5" : "mb-6 space-y-2";

    return (
      <div className={textStyle}>
        
        {/* LETTERHEAD */}
        <div className={`text-center ${isPdf ? "border-b-2 border-[#001232] pb-2 mb-4" : "border-b border-cobalt/30 pb-6 mb-8"}`}>
          ...
          <p className={`${isPdf ? "text-[9px]" : "text-xs"} font-bold uppercase tracking-widest`}>YUSDAAM Autos Fleet Management Nigeria Limited</p>
          <p className={`${isPdf ? "text-[8px]" : "text-[10px] text-slate-light"} mt-1`}>RC: 9335611 | 84, Addo Road, Oke-Ira Kekere, Ajah, Lagos | admin@yusdaamautos.com</p>
        </div>

        <h2 className={`text-center font-black uppercase ${isPdf ? "text-xs mb-4" : "text-lg text-signal-red mb-8"}`}>DRIVER/RIDER HIRE PURCHASE AGREEMENT</h2>
        ...
        <p className="mb-1 font-bold">BETWEEN</p>
        ...
        <p className="mb-1 font-bold">AND</p>
        ...
        <p className="mb-1 font-bold">AND</p>
        <p className={paraSpacing}><strong>THE GUARANTORS</strong>, whose names, addresses, and details are expressly set out in Clause 7 of this Agreement (hereinafter referred to as the <strong>"Guarantors"</strong>), of the third part.</p>

        <h3 className={headingStyle}>RECITALS</h3>
        <p className="mb-1 font-bold">WHEREAS:</p>
        ...
        <p className={`${paraSpacing} font-bold uppercase ${isPdf ? "bg-gray-200 p-1 border border-black" : "bg-gray-100 p-2 text-black border border-gray-300 inline-block"}`}>NOW, THEREFORE, IT IS HEREBY AGREED AS FOLLOWS:</p>

        <h3 className={headingStyle}>1. THE ASSET</h3>
        <p className="mb-1">1.1 The Administrator hereby agrees to let, and the Driver/Rider agrees to hire, the following commercial transport vehicle (the <strong>"Asset"</strong>):</p>
        <ul className={`list-none pl-4 ${listSpacing} ${isPdf ? "font-mono border border-black p-2 bg-gray-50 grid grid-cols-2 gap-x-4" : "font-mono bg-void-navy/50 p-4 rounded-lg"}`}>
        ...
        <p className={paraSpacing}>2.5 <strong>Tenure:</strong> The expected duration is <strong>{contract?.agreedDurationMonths ? contract.agreedDurationMonths * 4 : "---"}</strong> weeks, concluding when the Total Hire Purchase Price is fully paid.</p>
        ...
        <p className={paraSpacing}>3.5 <strong>GPS & Telematics:</strong> A GPS tracking device is actively installed on the Asset. Tampering with, disconnecting, obscuring, or damaging the GPS tracker is considered an act of theft, resulting in immediate repossession and criminal prosecution.</p>
        ...
        <p className="mb-1">4.2 <strong>Right of Repossession:</strong> The Administrator reserves the right to forcefully recover and repossess the Asset without prior court order, legal notice, or liability for trespass if:</p>
        ...
        <p className={paraSpacing}>5.4 <strong>Recovery Costs:</strong> If the Administrator is forced to employ asset recovery agents, legal counsel, or law enforcement to trace an absconded Driver/Rider, all tracking, legal, and recovery expenses shall be billed directly to the Guarantors.</p>
        ...
        <p className={paraSpacing}>6.3 <strong>Jurisdiction:</strong> This Agreement shall be governed by the laws of the Federal Republic of Nigeria. Any dispute shall be subject to the exclusive jurisdiction of the Courts of Lagos State.</p>

        {/* PAGE BREAK FOR PDF SO SIGNATURES ARE TOGETHER */}
        {isPdf && <div style={{ pageBreakBefore: 'always', paddingTop: '10px' }}></div>}

        <h3 className={headingStyle}>7. GUARANTORS' EXECUTION</h3>
        <div className={`${isPdf ? "mb-4 p-3 border border-black bg-gray-50" : "mb-8 p-4 border border-cobalt/30 bg-void-navy/50"}`}>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <div className="flex items-end gap-2 h-10">
                ...
                {g1?.signatureUrl ? <img src={g1.signatureUrl} alt="G1 Sig" className="h-8 object-contain" /> : <span className="border-b border-black w-16 inline-block"></span>}
              </div>
            ...
            <div>
              ...
              <div className="flex items-end gap-2 h-10">
                <span className={isPdf ? "text-[9px]" : "text-xs"}>Signature:</span>
                {g2?.signatureUrl ? <img src={g2.signatureUrl} alt="G2 Sig" className="h-8 object-contain" /> : <span className="border-b border-black w-16 inline-block"></span>}
              </div>
        ...
        <div className={`grid grid-cols-2 gap-6 ${isPdf ? "mb-6" : "mb-8"}`}>
          <div>
            <p className="font-bold mb-2 text-[10px]">SIGNED by the ADMINISTRATOR</p>
            <div className="relative h-12 w-40 mb-1">
               <img src="/images/stamp.png" alt="Company Seal" className="absolute left-0 -top-2 h-16 opacity-80 object-contain" />
          ...
          <div>
            <p className="font-bold mb-2 text-[10px]">SIGNED by the DRIVER/RIDER</p>
            <div className="relative h-12 w-40 mb-1">
              {riderSig ? (
                <img src={riderSig} alt="Rider Signature" className={`absolute left-0 bottom-0 h-12 object-contain ${!isPdf ? "bg-white p-1 rounded" : "mix-blend-multiply"}`} />
              ) : (
                <div className={`absolute bottom-0 w-full border-b ${isPdf ? "border-black" : "border-slate-light"}`}></div>
              )}
            </div>
        ...
        <div className="mb-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div className="relative h-12 w-40">
              ...
              {witnessSig ? (
                <img src={witnessSig} alt="Witness Signature" className={`absolute left-14 bottom-0 h-12 object-contain ${!isPdf ? "bg-white p-1 rounded" : "mix-blend-multiply"}`} />
              ) : (
                <div className={`absolute bottom-0 left-12 w-full border-b ${isPdf ? "border-black" : "border-slate-light"}`}></div>
              )}
            </div>
          </div>
        </div>
    ...
    <div className="fixed inset-0 z-[100] bg-void-navy/95 backdrop-blur-md overflow-y-auto h-screen w-screen px-2 sm:px-6 py-10">
      <div ref={topRef} className="max-w-5xl mx-auto bg-void-light/5 border border-cobalt/30 rounded-xl shadow-2xl animate-in slide-in-from-bottom-8 duration-500 w-full overflow-hidden relative">
        ...
        <div className="absolute top-[-10000px] left-[-10000px]">
          <div ref={pdfContractRef} className="bg-white p-10 w-[800px]">
