...
import { 
  ...
  ChevronDown, ChevronUp, UserCheck, UserX, FileText, User, Search, ChevronLeft, ChevronRight, Trash2, Download 
} from "lucide-react";
...
const DocumentPreview = ({ url, label, personName = "Rider" }: { url: string, label: string, personName?: string }) => {
  if (!url) return null;
...
export default function RidersKycClient({ riders }: { riders: Rider[] }) {
  ...
  return (
    <div className="space-y-4">
      ...
      {/* RIDERS LIST (PAGINATED - 10 PER PAGE) */}
      ...
      {filteredRiders.length > 0 && (
        <div className="p-3.5 border border-white/10 rounded-xl bg-[#0e1626] flex flex-wrap items-center justify-between gap-3 text-xs">
          <span className="text-gray-400">
            Showing <strong className="text-white">{(page - 1) * ITEMS_PER_PAGE + 1}</strong> to <strong className="text-white">{Math.min(page * ITEMS_PER_PAGE, filteredRiders.length)}</strong> of <strong className="text-white">{filteredRiders.length}</strong> riders
          </span>
      ...
      {filteredRiders.length === 0 && (
        <div className="p-8 text-center text-gray-500 text-xs bg-[#0e1626] rounded-xl border border-white/10">
          <User size={32} className="mx-auto text-gray-600 mb-2 opacity-50" />
          <p>No riders found.</p>
        </div>
      )}
